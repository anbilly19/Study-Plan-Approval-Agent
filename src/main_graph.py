...
from state import StudyPlanEvaluation, StudyPlanState
...
def _run_with_hitl(app, initial_state: StudyPlanState) -> dict:
    ...
    if "__interrupt__" in result:
        ...
        if decision == "edit":
            ...
            human_response = {"action": "edit", "data": override_scores}
...
def _print_evaluation_summary(result: dict) -> None:
    ...
    if result.get("final_evaluation"):
        eval_obj = (
            ...
            if isinstance(result["final_evaluation"], StudyPlanEvaluation)
            else result["final_evaluation"]["structured_response"]
        )
