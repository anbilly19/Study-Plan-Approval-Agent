...
class WorkloadScoreInput(BaseModel):
    num_courses: int = Field(description="The number of courses in the study plan")

@tool(args_schema=WorkloadScoreInput)
...
class DatabaseTool:
    ...
    def tool_factory(self) -> dict[str, StructuredTool]:
        ...
        for name, func in self.df_funcs.items():
            ...
            # Use inner function to preserve signature/schema for OpenAI
            # We need to capture 'func' in the closure and define arguments clearly.
            # Using a helper to create the closure properly.
            def make_query_tool(captured_func=func):
                def query_tool(query: str) -> str:
                    return captured_func(query)
                return query_tool
            
            tool_fn = make_query_tool(func)
